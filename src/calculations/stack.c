...
int is_empty_stack(stack *operators) {  // проверка стека на пустоту, 1 - стек
                                        // пустой, 0 - непустой
  int status = 0;
  if (operators->top == -1) {
    status = 1;
...
int is_full_stack(stack *operators) {  // проверка стека на заполненность, 1 -
                                       // стек полон, 0 - не полон
  int status = 1;
...
//         operators->items[i].type, operators->items[i].lexeme_kind,
//         operators->items[i].number, operators->items[i].sign,
//         operators->items[i].unary);
//     }
// }
...
//     for (int i = 0; i < 13; i++) {
//         printf("элемент %d", i);
//         printf(": type = %d kind = %d number = %lf unary = %d\n",
//         arr[i].type, arr[i].lexeme_kind, arr[i].number, arr[i].unary);
//     }
// }
